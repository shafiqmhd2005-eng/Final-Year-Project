...
const operations = [
  {
    icon: Heart,
    title: "Interactive Cardiac Simulation",
    description: "Control heart rate, blood pressure, and other vital parameters to observe physiological responses in a risk-free digital environment",
    color: "red",
    delay: "delay-500"
  },
  {
    icon: Activity,
    title: "Hemodynamic Analysis",
    description: "Examine cardiovascular performance, blood flow dynamics, and circulatory efficiency with detailed visualizations and metrics",
    color: "pink",
    delay: "delay-600"
  },
  {
    ...
    description: "Study ECG patterns, heart sounds, valvular function, and arrhythmia detection across various physiological conditions",
    color: "green",
    delay: "delay-700"
  },
  {
    ...
    description: "Monitor autonomic nervous system effects, stress impacts, and brain-heart communication pathways on cardiac function",
    color: "purple",
    delay: "delay-800"
  }
];
    ...
    <div className={`w-full max-w-6xl mb-8 sm:mb-12 px-2 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'} delay-400`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        {operations.map((item, index) => (
          <div
            key={index}
            className={`bg-gradient-to-br from-white/5 to-${item.color}-500/10 backdrop-blur-md rounded-xl p-4 sm:p-6 border border-white/10 hover:border-${item.color}-400/30 hover:bg-${item.color}-500/5 transition-all duration-500 animate-slide-up ${item.delay} group`}
          >
            <div className="flex items-start gap-3 sm:gap-4">
              <div className={`p-2 sm:p-3 bg-${item.color}-500/20 rounded-lg mt-1 group-hover:scale-110 transition-transform duration-300`}>
                <item.icon className={`w-5 h-5 sm:w-6 sm:h-6 text-${item.color}-400`} />
